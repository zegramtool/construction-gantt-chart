...
### 8. レスポンシブ対応
- PC版（768px以上）: 5列表示（工程名｜担当｜開始｜終了｜チャート）
- スマホ・iPhone版（767px以下）: 2列表示（工程｜チャート）
  - 1段目: 工程名 + 色ボタン + ドラッグハンドル
  - 2段目: 担当・開始・終了を3列グリッドで横スクロール不要で入力
  - 担当・開始・終了列は非表示（メディアクエリで強制非表示）
- タスク色変更: 12色パレット、スマホでは色ボタン拡大・パネルは下に表示
...
- 時間入力の最適化（数値入力キーボード）
- スマホ・iPhone: 2列レイアウト（工程ブロック｜チャート）、横スクロール不要で入力可能
- 色変更UI: スマホで色ボタン拡大（w-5 h-5）、パネルは下方向に表示
- index.css の @media (max-width: 767px) で列の表示制御（iOS Safari対応）
...
## 更新履歴
- 2026-02-01: スマホ・iPhone対応強化
  - チャートカレンダーをスマホで2列レイアウト（工程｜チャート）に変更
  - 2段表示: 1段目=工程名+色+ドラッグ、2段目=担当・開始・終了（横スクロール不要）
  - 色変更UIをスマホで押しやすく（ボタン拡大、パネル下表示）
  - index.css に @media (max-width: 767px) を追加しiPhoneで確実に2列表示
- 2026-01-27: 初版作成
  - 時間スケール（5分刻み）実装
  - スケール別の独立した値管理
  - モバイル対応の時間入力UI
